//
// Created by Sijia Chen on 2020/5/1.
//
...
DictionaryRef DictionaryCreate( size_t size, DictionaryCallbacks * callbacks )
{
    ...
    dictionary_ref = calloc( sizeof( struct Dictionary ), 1 );
    items_pointer = calloc( sizeof( struct DictionaryItem * ), size );
    ...
    if( dictionary_ref->count ){
        for( int i = 0;i < dictionary_ref->size; i++ )
        {
            ...
            while( item ){
                ...
                printf(" %s | value : %d;", item->key, *same_messages_received_number);
                item = item->next;
            }
        }
    }
